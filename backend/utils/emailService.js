...
const sendVerificationEmail = async (email, name, otp) => {
  ...
  const mailOptions = {
    from: `"Kagoj" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: 'Verify your Kagoj account',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 8px;">
        ...
        <p style="color: #6b7280; font-size: 14px;">If you didn't create an account on Kagoj, you can safely ignore this email.</p>
        <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 24px 0;" />
        <p style="color: #9ca3af; font-size: 12px; text-align: center;">&copy; Kagoj — Your E-Commerce Destination</p>
      </div>
    `,
  };
