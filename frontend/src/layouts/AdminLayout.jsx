...
const AdminLayout = () => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      {/* Sidebar */}
      <aside className="fixed inset-y-0 left-0 w-64 bg-white border-r border-gray-200 flex flex-col overflow-y-auto z-20">
        {/* Logo */}
        <div className="p-5 border-b border-gray-100">
          <Link to="/" className="text-xl font-bold text-indigo-600 hover:text-indigo-700 transition">Kagoj</Link>
        ...
        <nav className="flex-1 p-4 space-y-4">
          {NAV_SECTIONS.map((section) => (
            <div key={section.label}>
              <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-2 px-3">
                {section.label}
              </p>
      ...
      <div className="ml-64 flex-1 flex flex-col overflow-hidden">
        {/* Top bar */}
        <header className="sticky top-0 z-10 bg-white border-b border-gray-200 h-16 px-6 flex items-center justify-between shrink-0">
